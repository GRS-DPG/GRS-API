...
<head>
  <meta charset="utf-8">
  <title>Claim 1 Bill Trainee List</title>
  <style>
    .clearfix:after {
      content: "";
      display: table;
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td style="text-align:left">{{$value['batch_info']['batch_sequence_number']}}</td>
          <td style="text-align:left">@if($value['batch_info']['active_status'] == 1) Active @else Inactive @endif</td>
          <td style="text-align:left">
            {{$value['trainee_count']}}
          </td>
        ...
        <tr>
          ...
          <td>&nbsp;</td>
          <td align="center">{{$bill_info['total_trainee']}}</td>
          <td colspan="3" align="right"><b>A) Gross Amount</b></td>
          <td>{{number_format($bill_info['gross_amount'], 2)}}</td>
        </tr>
        <tr>
          <td colspan="9" align="right"><b>B) IA\'S Contribution</b></td>
          <td>{{$bill_info['contribution_percentage']}} % </td>
        </tr>
        <tr>
          <td colspan="9" align="right"><b>C) Contribution Amount (A*B)</b></td>
          <td>{{number_format($bill_info['contribution_amount'], 2)}}</td>
        </tr>
        <tr>
          <td colspan="9" align="right"><b>D) Net Payable (A - C)</b></td>
          <td>{{number_format($bill_info['net_amount'], 2)}}</td>
        </tr>
      </tbody>
    </table>
